...
3. **TRL 판정 근거 필수**: 매트릭스 또는 본문에 각 TRL 판정의 근거(양산 발표, 특허, 어닝콜, 채용 공고 등)가 명시되어 있는가? TRL 숫자만 있고 근거가 없으면 NOT_OK.
4. TRL 4~6 간접 추정 항목에 "⚠ 간접 추정" 표시와 추정근거(estimation_basis)가 명시되었는가?
5. 전략적 시사점이 단순 정보 나열이 아닌 R&D 대응 방향 제언을 포함하는가?
6. 모든 시사점에 출처 번호([1], [2], ...)가 연결되어 있는가?
